...
addpath('guided_design_group_fxns/')
...
% need be.
global dataTable iter file_name tableHeaders noteCell note_file timeTrack

group_name = "computer_test_1";

% Main parts of the table/calculation stuff for it (" is for those that
% need to be appended, those that 
[cost_vals, ~, materialsHeaders, Amount, entry ] = shared_tables();

tableHeaders = {'Iteration', 'Feasibility','Marbles', 'Range','Cost','Fleet_Size', 'Steps','PercentEffective','iterTime'}';
tableHeaders = [tableHeaders;materialsHeaders]';

file_name = "data_guided_group_"+group_name+".xls";
note_file = "notes_guided_group_"+group_name+".xls";
...
% I am not sure how else to pass data out of the run. It's not efficient but it works
 ...
 if isfile(file_name)
    ...
    dataTable = readtable(file_name,'Sheet',sheet(end),'EmptyValue',0);
    %the number of feasibility entries determines the iterations. 
    
    past_iter = sum(dataTable.Feasibility==1) + sum(dataTable.Feasibility==-1);
    iter = past_iter + 1;
    %there's some issue here regarding length of the table if you close it
    %at the wrong time. I think that's fixed now with the fact that I fixed
    %feasibility feedback 
 else
    %init_samps = [4, 350; 10, 220; 16, 280];
    %init_samps = [4, 300; 10, 220; 16, 280];
    init_samps = [4, 350;8, 220; 12, 280];
    c = zeros(3, 21);
    c(:, 1) = (1:3)';
    c(:,3:4) = init_samps;
    dataTable = array2table(c,'VariableNames', tableHeaders);
    iter = 1;
    writetable(dataTable,file_name,'Sheet',num2str(0));
 end

 if isfile(note_file)
     sheet = sheetnames(note_file);
     noteCell= readcell(note_file,'Sheet',sheet(end));
...
fig = uifigure('Name', 'Interactive Fleet Designer', 'Position', [100 100 1200 600]);
g = uigridlayout(fig);

g.RowHeight = {'1x', '12x', '6x'};
g.ColumnWidth = {'2x', '3x'};
...
global goalInfo
goalInfo = uilabel(sg1, "Text",['Current Design Goal: ', ...
    num2str(dataTable.Marbles(iter)), ' Marbles and ', num2str(dataTable.Range(iter)), ' Range']);
...
% let's add in the secret label here for the timer refresh
...
%nextButton = uibutton(sg2, 'Text', 'Next Design','ButtonPushedFcn',@(btn, event) nextDesign());
%nextButton.Layout.Row = 2; nextButton.Layout.Column =3;

%run button changed to entry button. Will turn into run simulation later
runButton = uibutton(sg2, 'push','Text',sprintf('Design entered and\nverified.') ,...
    'ButtonPushedFcn', @(btn, event) pushRunButton('guided',marbLamp, rangeLamp, wheelLamp, marblesEntry, entryTable, cost, path, cost_vals, simFig,resultsLabel,rangeDisp, costDisp, fleetDisp));
...
% start the design timer (readd this in after meeting on wednesday)
timeTrack = tic;
