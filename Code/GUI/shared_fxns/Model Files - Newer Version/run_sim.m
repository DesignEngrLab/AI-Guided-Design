function steps_to_completion = run_sim(num_agents, num_seeds, range, jobTime, cost, path)
...
max_steps = 1250; 
steps_to_completion = max_steps; %max step 
...
for i = 1:max_steps % Switch to while loop later, this is safer for testing
    ...
    %
    plot(plot_agent(:,1), plot_agent(:,2),'o','MarkerFaceColor','r', 'MarkerEdgeColor','y')
    ...
    if ~isempty(plot_tree)
        plot(plot_tree(:,1), plot_tree(:,2),'o', 'Color','w')
    end
