...
# %%
track_if_invalid = [i > 0 for i in track_total_invalids]

print(
    sum(track_if_invalid),
    len(track_if_invalid),
    sum(track_if_invalid) / len(track_if_invalid),
)

print(
    sum(track_total_invalids),
    sum(total_cons),
    sum(track_total_invalids) / sum(total_cons),
)

# %%
checking_specifics = []
for i in range(0, len(track_if_invalid)):
    if track_if_invalid[i]:
        checking_specifics.append(track_total_invalids[i] / total_cons[i])
